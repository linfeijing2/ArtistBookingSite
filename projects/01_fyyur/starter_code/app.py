...
def venues():
  ...
  # all_areas = Venue.query(Venue.state,Venue.city,func.count(Venue.id)).group_by(Venue.state,Venue.city).all()
...
def show_venue(venue_id):
  ...
  upcoming_shows_query = db.session.query(Show).filter(Show.venue_id == venue_id).filter(Show.start_time > datetime.now()).all()
  past_shows_query = db.session.query(Show).filter(Show.venue_id == venue_id).filter(Show.start_time < datetime.now()).all()
  upcoming_shows = []
  past_shows = []
  for upcoming in upcoming_shows_query:
    upcoming_shows.append({
      "artist_id": upcoming.id,
      "artist_name": upcoming.name,
      "artist_image_link": upcoming.artist_image_link,
  ...
  for past in past_shows_query:
    past_shows.append({
      ...
      "artist_image_link": past_shows.artist_image_link,
...
def show_artist(artist_id):
  ...
  if not artist: 
    return render_template('errors/404.html')
  upcoming_shows_query = db.session.query(Show).filter(Show.artist_id == artist_id).filter(Show.start_time > datetime.now()).all()
  past_shows_query = db.session.query(Show).filter(Show.artist_id == artist_id).filter(Show.start_time < datetime.now()).all()
  upcoming_shows = []
  past_shows = []
  for upcoming in upcoming_shows_query:
    upcoming_shows.append({
      "venue_id": upcoming.id,
      "venue_name": upcoming.name,
      "venue_image_link": upcoming.venue_image_link,
      "start_time": upcoming.start_time
    })
  for past in past_shows_query:
    past_shows.append({
      ...
      "venue_image_link": past_shows.venue_image_link,
      "start_time": past_shows.start_time
    })
  data = {
    "id": artist.id,
...
def create_artist_submission():
  ...
  try: 
    ...
    flash('Artist ' + request.form['name'] + ' closed!')
  # on successful db insert, flash success
  # TODO: on unsuccessful db insert, flash an error instead.
